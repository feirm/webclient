<template>
  <div class="flex p-3 md:p-8 w-full bg-gray-200">
    <div class="container md:w-2/5 mx-auto space-y-4">
      ...
      <div class="p-6 shadow bg-white rounded space-y-3">
        ...
        <p>
          When using the Feirm Platform, two-factor authentication is mandatory.
          We allow you to login through a magic link sent to your email, or
          through a code generated by an OTP app such as Google Authenticator.
        </p>

        <p>Password: •••••••••••</p>
        <p>
          Two Factor Authentication:
          {{ determineTwoFactor(profile.two_factor_method) }}
        </p>
      ...
      <div class="p-6 shadow bg-white rounded space-y-3">
        ...
        <p>
          It can be an inconvenience having to login to your Feirm account every
          time you open the app. By saving your account key to this device, you
          can skip the login step when you next open the app.
        </p>
        ...
        <div class="mt-8 flex justify-left">
          <div class="inline-flex rounded-md shadow">
            <button
              v-if="!hasRootKey"
              @click="showDeviceSecurityModal = !showDeviceSecurityModal"
              class="inline-flex items-center justify-center px-5 py-2 rounded text-sm font-medium text-yellow-900 bg-orange-500 hover:bg-orange-400"
            >
              Save to device
            </button>
          </div>
  ...
  <div
    v-if="showModal"
    class="flex fixed h-screen justify-center inset-0 items-center"
  >
    <div class="z-50 md:w-1/3 relative bg-white p-8 rounded shadow-2xl">
      <div v-if="changeTwoFactor.step === 0" class="space-y-4">
        ...
        <div
          v-if="
            changeTwoFactor.selected !== profile.two_factor_method &&
              changeTwoFactor.selected == 'totp'
          "
        ...
        <div
          v-if="
            changeTwoFactor.selected !== profile.two_factor_method &&
              changeTwoFactor.selected == 'email'
          "
          class="p-3 rounded bg-yellow-200"
        >
          <p>
      ...
      <div v-if="changeTwoFactor.step === 1" class="space-y-4">
        <h1 class="text-2xl">
          Scan the QR code below into your authenticator app
        </h1>
        <img class="mx-auto" :src="totp.qrCode" />

        <div
          class="border-2 border-gray-200 rounded p-2 w-2/4 text-center mx-auto"
        >
        ...
        <div class="space-x-3">
          <button
            @click="submitTotp"
            class="p-3 rounded shadow bg-green-400 w-24 text-gray-900"
          >
            Submit
          </button>
          <button
      ...
      <div v-if="changeTwoFactor.step === 2" class="space-y-4">
        ...
        <p>
          Please take the time now to write down your recovery codes. These are
          important as they can help you can access to your account if you lose
          your two-factor authentication device.
        </p>
        <p>
          Feirm cannot change the two-factor authentication method or recover
        ...
        <button
          ...
          class="block w-full p-3 rounded shadow bg-orange-500 text-yellow-900"
        >
          I have written down my recovery codes
        </button>
      </div>
    </div>
