<template>
    <div class="flex flex-col bg-gray-100">
        <div class="w-full md:w-2/5 p-12 space-y-4 mx-auto">
            ...
            <p>Below you will see your non-custodial cryptocurrency wallet. This means that you are in total control of your private keys - they never leave your device. Feirm cannot access the funds on your behalf, meaning it is important to keep this mnemonic safe!</p>
            ...
            <div class="flex w-3/4 p-2 bg-gray-200 rounded justify-center mx-auto">
                ...
                <div class="flex-col p-2">
                    <div v-for="(word, index) in splitMnemonic" :key="word">
                        <code v-if="index >= 8 && index < 16">{{ index + 1 }}. {{ word }}</code>
                    </div>
                </div>
                ...
                <div class="flex-col p-2">
                    <div v-for="(word, index) in splitMnemonic" :key="word">
                        <code v-if="index >= 16 && index < 24">{{ index + 1 }}. {{ word }}</code>
                    </div>
                </div>
            </div>

            <!-- Option for encrypted backup -->
            <p>Do you frequently find yourself changing devices? Having to restore your mnemonic every time can be a time consuming process. You can store an encrypted copy of your wallet which is linked to your Feirm account, and access your funds immediately when you sign into a new device.</p>
            <label class="flex items-center justify-center">
                <input type="checkbox" class="form-checkbox" v-model="cloudBackup">
                <span class="ml-2">Upload wallet to my <span class="text-orange">Feirm</span> account.</span>
            </label>
