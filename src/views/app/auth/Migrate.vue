          ...
          <div class="mt-6">
            <form @submit.prevent="migrateRequest" class="space-y-6">
              ...
              <div class="space-y-1">
                ...
                <div class="mt-1">
                  <input name="pin" type="password" v-model="pin" autocomplete="current-password" required class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-orange-500 focus:border-orange-500 sm:text-sm" />
                </div>
              </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    async migrateRequest() {
      ...
      let oldAccount;
      try {
          await tatsuyaService.fetchEncryptedAccount(this.username, this.pin).then(res => {
              oldAccount = res.data;
      ...
      let rootKey;
      try {
        const salt = utils.hex.toBytes(oldAccount.encryptedRootKey.iv).slice(0, 16);
        const aesCbc = new ModeOfOperation.cbc(encryptionKey, salt);

        rootKey = aesCbc.decrypt(utils.hex.toBytes(oldAccount.encryptedRootKey.cipherText));
      } catch (e) {
          this.submitted = false;
          return this.$toast.error(e);
      }
